import { Link } from 'react-router-dom';
import { ArrowLeft, Users, ShoppingCart, DollarSign, TrendingUp, X, Sparkles } from 'lucide-react';
import Chart from 'react-apexcharts';
import { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { useTheme } from '../../application/hooks/useTheme';
import { Button } from '../components/ui/Button';

// ─── helper ───────────────────────────────────────────────────────────────────
const pct = (prev, curr) => {
  const val = ((curr - prev) / prev) * 100;
  return val.toFixed(2);
};

const weeks = ['M12→M13', 'M13→M14', 'M14→M15', 'M15→M16'];

// ─── raw data ─────────────────────────────────────────────────────────────────
const customerRaw = {
  Customer: [7, 7, 8, 9, 17],
  Order:    [8, 8, 9, 10, 18],
  Omset:    [167000, 167000, 181000, 230000, 422000],
};

const buildChanges = (raw) =>
  Object.fromEntries(
    Object.entries(raw).map(([key, vals]) => {
      const changes = [];
      for (let i = 0; i < vals.length - 1; i++) {
        changes.push({
          pct: pct(vals[i], vals[i + 1]),
          abs: vals[i + 1] - vals[i]
        });
      }
      return [key, changes];
    })
  );

const customerChanges = buildChanges(customerRaw);

const getPeakGrowth = (raw) => {
  const changes = buildChanges(raw);
  const peaks = {};

  Object.entries(changes).forEach(([metric, wVals]) => {
    let maxVal = -Infinity;
    let maxIdx = 0;
    wVals.forEach((val, idx) => {
      const pctNum = parseFloat(val.pct);
      if (pctNum > maxVal) {
        maxVal = pctNum;
        maxIdx = idx;
      }
    });
    peaks[metric] = {
      week: weeks[maxIdx],
      pct: maxVal,
      abs: wVals[maxIdx].abs,
    };
  });

  return peaks;
};

// ─── sub-component: collapsible analysis panel ───────────────────────────────
const AnalyticsPanel = ({ changes, accentColor, isOpen, onClose, isCurrency }) => {
  const paramColors = {
    Customer: 'text-violet-500',
    Order:    'text-emerald-500',
    Omset:    'text-rose-500',
  };

  return (
    <AnimatePresence initial={false}>
      {isOpen && (
        <motion.div
          initial={{ height: 0, opacity: 0, marginTop: 0 }}
          animate={{ height: 'auto', opacity: 1, marginTop: 12 }}
          exit={{ height: 0, opacity: 0, marginTop: 0 }}
          transition={{ duration: 0.3, ease: 'easeInOut' }}
          className="overflow-hidden"
        >
          <div className="rounded-2xl border border-[var(--color-border-custom)] bg-[var(--color-bg-card)] shadow-sm">
            {/* panel header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border-custom)]">
              <span className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider flex items-center gap-1.5">
                <TrendingUp size={13} className={accentColor} />
                persentase per minggu
              </span>
              <button
                onClick={onClose}
                className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-[var(--color-text-secondary)] cursor-pointer"
                aria-label="Tutup panel"
              >
                <X size={14} />
              </button>
            </div>

            {/* tabel persentase */}
            <div className="p-4 overflow-x-auto">
              <table className="w-full text-xs min-w-[320px]">
                <thead>
                  <tr>
                    <th className="text-left text-[var(--color-text-secondary)] font-semibold pb-2 pr-3 w-1/4">Parameter</th>
                    {weeks.map((w) => (
                      <th key={w} className="text-center text-[var(--color-text-secondary)] font-semibold pb-2 px-1">{w}</th>
                    ))}
                    <th className="text-center text-[var(--color-text-secondary)] font-semibold pb-2 px-1 pl-3 border-l border-[var(--color-border-custom)]">Rata-rata</th>
                  </tr>
                </thead>
                <tbody>
                  {Object.entries(changes).map(([param, vals]) => (
                    <tr key={param} className="border-t border-[var(--color-border-custom)]">
                      <td className={`py-2.5 pr-3 font-semibold ${paramColors[param] ?? 'text-[var(--color-text-primary)]'}`}>
                        {param}
                      </td>
                      {vals.map(({ pct: pctVal, abs: absVal }, i) => {
                        const num = parseFloat(pctVal);
                        const isUp = num >= 0;
                        const displayAbs = isCurrency ? 'Rp ' + absVal.toLocaleString('id-ID') : absVal.toLocaleString('id-ID');
                        return (
                          <td key={i} className="py-2.5 px-1 text-center">
                            <span className={`inline-flex flex-col items-center justify-center font-medium rounded-md px-2 py-1
                              ${isUp
                                ? 'bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-400'
                                : 'bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400'
                              }`}>
                              <span className="flex items-center gap-0.5 whitespace-nowrap">
                                {isUp ? '▲' : '▼'} {Math.abs(num).toFixed(2)}%
                              </span>
                              <span className="text-[10px] opacity-75 font-normal whitespace-nowrap">
                                ({isUp ? '+' : ''}{displayAbs})
                              </span>
                            </span>
                          </td>
                        );
                      })}
                      {(() => {
                        const avgPct = vals.reduce((acc, curr) => acc + parseFloat(curr.pct), 0) / vals.length;
                        const avgAbs = vals.reduce((acc, curr) => acc + curr.abs, 0) / vals.length;
                        const isUp = avgPct >= 0;
                        const displayAvgAbs = isCurrency ? 'Rp ' + avgAbs.toLocaleString('id-ID', { maximumFractionDigits: 1 }) : avgAbs.toLocaleString('id-ID', { maximumFractionDigits: 1 });
                        return (
                          <td className="py-2.5 px-1 pl-3 text-center border-l border-[var(--color-border-custom)] bg-gray-50/20 dark:bg-gray-800/5">
                            <span className={`inline-flex flex-col items-center justify-center font-semibold rounded-md px-2 py-1
                              ${isUp
                                ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/30'
                                : 'bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 border border-rose-100 dark:border-rose-900/30'
                              }`}>
                              <span className="flex items-center gap-0.5 whitespace-nowrap">
                                {isUp ? '▲' : '▼'} {Math.abs(avgPct).toFixed(2)}%
                              </span>
                              <span className="text-[10px] opacity-75 font-normal whitespace-nowrap">
                                ({isUp ? '+' : ''}{displayAvgAbs})
                              </span>
                            </span>
                          </td>
                        );
                      })()}
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          </div>
        </motion.div>
      )}
    </AnimatePresence>
  );
};

// ─── sub-component: KPI platform card ──────────────────────
const PlatformCard = ({ accentClass, label, value, icon, bgAccent, changeVal, pctVal, isCurrency }) => {
  const isUp = parseFloat(pctVal) >= 0;
  const displayVal = isCurrency ? 'Rp ' + value.toLocaleString('id-ID') : value.toLocaleString('id-ID');
  const displayChange = isCurrency ? 'Rp ' + Math.abs(changeVal).toLocaleString('id-ID') : Math.abs(changeVal).toLocaleString('id-ID');

  return (
    <div className="rounded-2xl border border-[var(--color-border-custom)] bg-[var(--color-bg-card)] p-5 shadow-sm flex items-center justify-between">
      <div>
        <span className={`text-xs font-semibold block mb-0.5 ${accentClass}`}>{label}</span>
        <h4 className="text-2xl font-bold font-display">{displayVal}</h4>
        <div className="flex items-center gap-1 mt-1 text-[11px]">
          <span className={isUp ? 'text-green-600 dark:text-green-400 font-semibold' : 'text-red-600 dark:text-red-400 font-semibold'}>
            {isUp ? '▲' : '▼'} {pctVal}%
          </span>
          <span className="text-[var(--color-text-secondary)]">
            ({isUp ? '+' : '-'}{displayChange}) vs M15
          </span>
        </div>
      </div>
      <div className={`p-2.5 rounded-xl ${bgAccent}`}>
        {icon}
      </div>
    </div>
  );
};

// ─── sub-component: chart card with analysis button ─────────────────────────
const ChartCard = ({ title, subtitle, icon, series, chartOptions, changes, accentClass, isCurrency }) => {
  const [open, setOpen] = useState(false);

  return (
    <div className="flex flex-col">
      <div className="rounded-2xl border border-[var(--color-border-custom)] bg-[var(--color-bg-card)] p-4 sm:p-6 shadow-sm flex flex-col justify-between">
        <div>
          <div className="pb-4 mb-4 border-b border-[var(--color-border-custom)] flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold font-display text-[var(--color-text-primary)] flex items-center gap-2">
                {icon} {title}
              </h3>
              <p className="text-xs text-[var(--color-text-secondary)] mt-1">{subtitle}</p>
            </div>
            <button
              onClick={() => setOpen((o) => !o)}
              className={`flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1.5 rounded-lg border transition-colors whitespace-nowrap cursor-pointer
                ${open
                  ? 'bg-[var(--color-bg-primary)] border-[var(--color-border-custom)] text-[var(--color-text-secondary)]'
                  : 'border-[var(--color-border-custom)] hover:bg-gray-50 dark:hover:bg-gray-800 text-[var(--color-text-secondary)]'
                }`}
            >
              <TrendingUp size={11} className={accentClass} />
              {open ? 'Tutup' : 'Analisis'}
            </button>
          </div>

          <div className="h-[300px] w-full">
            <Chart
              options={chartOptions}
              series={series}
              type="line"
              height="100%"
              width="100%"
            />
          </div>
        </div>
      </div>

      <AnalyticsPanel
        changes={changes}
        accentColor={accentClass}
        isOpen={open}
        onClose={() => setOpen(false)}
        isCurrency={isCurrency}
      />
    </div>
  );
};

// ─── sub-component: customer peak growth card ─────────────────────────────────
const CustomerPeakGrowthCard = ({ peaks, theme }) => {
  const formatWeek = (weekStr) => {
    return weekStr.replace('→', ' ke ');
  };

  const categories = Object.keys(peaks);
  const seriesData = categories.map((metric) => peaks[metric].pct);

  const chartSeries = [
    {
      name: 'Puncak Pertumbuhan',
      data: seriesData,
    },
  ];

  const chartOptions = {
    chart: {
      type: 'bar',
      toolbar: { show: false },
      zoom: { enabled: false },
      foreColor: theme === 'dark' ? '#94a3b8' : '#64748b',
      background: 'transparent',
    },
    colors: ['#a855f7', '#10b981', '#f43f5e'],
    plotOptions: {
      bar: {
        horizontal: true,
        barHeight: '60%',
        borderRadius: 4,
        distributed: true,
      },
    },
    dataLabels: {
      enabled: true,
      formatter: (val) => `${val.toFixed(1)}%`,
      style: {
        fontSize: '9px',
        fontFamily: 'Plus Jakarta Sans, sans-serif',
        colors: ['#ffffff'],
      },
      dropShadow: {
        enabled: false,
      },
    },
    grid: {
      borderColor: theme === 'dark' ? '#334155' : '#e2e8f0',
      strokeDashArray: 4,
      xaxis: { lines: { show: true } },
      yaxis: { lines: { show: false } },
    },
    xaxis: {
      categories: categories,
      labels: {
        formatter: (val) => `${val}%`,
        style: {
          fontSize: '9px',
        },
      },
      axisBorder: { show: false },
      axisTicks: { show: false },
    },
    yaxis: {
      labels: {
        style: {
          fontSize: '10px',
          fontFamily: 'Plus Jakarta Sans, sans-serif',
          fontWeight: 600,
        },
      },
    },
    tooltip: {
      theme: theme === 'dark' ? 'dark' : 'light',
      y: {
        formatter: (val, { dataPointIndex }) => {
          const metric = categories[dataPointIndex];
          const item = peaks[metric];
          const displayAbs = metric === 'Omset' ? 'Rp ' + item.abs.toLocaleString('id-ID') : item.abs.toLocaleString('id-ID');
          return `${val.toFixed(2)}% (Puncak: ${formatWeek(item.week)} · +${displayAbs})`;
        },
      },
    },
    legend: {
      show: false,
    },
  };

  return (
    <div className="space-y-4">
      <div className="h-[200px] w-full">
        <Chart
          options={chartOptions}
          series={chartSeries}
          type="bar"
          height="100%"
          width="100%"
        />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {Object.entries(peaks).map(([metric, data]) => {
          const displayAbs = metric === 'Omset' ? 'Rp ' + data.abs.toLocaleString('id-ID') : data.abs.toLocaleString('id-ID');
          return (
            <div key={metric} className="p-3 rounded-xl bg-gray-50/50 dark:bg-gray-800/20 border border-[var(--color-border-custom)]/40 text-xs">
              <div className="flex justify-between items-center mb-1">
                <span className="font-semibold text-[var(--color-text-secondary)]">{metric}</span>
                <span className="font-bold text-green-600 dark:text-green-400 flex items-center gap-0.5">
                  ▲ {data.pct}%
                </span>
              </div>
              <div className="text-[var(--color-text-secondary)] flex justify-between">
                <span>Puncak: {formatWeek(data.week)}</span>
                <span className="font-medium">({data.abs > 0 ? `+${displayAbs}` : displayAbs})</span>
              </div>
            </div>
          );
        })}
      </div>
    </div>
  );
};

// ─── main component ────────────────────────────────────────────────────────────
export const CustomerDashboard = () => {
  const { theme } = useTheme();

  const customerSeries = [
    { name: 'Customer', data: customerRaw.Customer }
  ];

  const orderSeries = [
    { name: 'Order', data: customerRaw.Order }
  ];

  const omsetSeries = [
    { name: 'Omset', data: customerRaw.Omset }
  ];

  const getChartOptions = (colors, isCurrency = false) => ({
    chart: {
      type: 'line',
      toolbar: { show: false },
      zoom: { enabled: false },
      foreColor: theme === 'dark' ? '#94a3b8' : '#64748b',
      background: 'transparent',
    },
    colors,
    stroke: { curve: 'smooth', width: 3 },
    grid: {
      borderColor: theme === 'dark' ? '#334155' : '#e2e8f0',
      strokeDashArray: 4,
      xaxis: { lines: { show: false } },
      yaxis: { lines: { show: true } },
    },
    xaxis: {
      categories: ['Minggu 12', 'Minggu 13', 'Minggu 14', 'Minggu 15', 'Minggu 16'],
      axisBorder: { show: false },
      axisTicks: { show: false },
    },
    yaxis: {
      labels: {
        formatter: (val) => isCurrency ? 'Rp ' + val.toLocaleString('id-ID') : val.toLocaleString('id-ID')
      },
    },
    tooltip: {
      theme: theme === 'dark' ? 'dark' : 'light',
      y: {
        formatter: (val, { seriesIndex, dataPointIndex, w }) => {
          const displayVal = isCurrency ? 'Rp ' + val.toLocaleString('id-ID') : val.toLocaleString('id-ID');
          if (dataPointIndex === 0) {
            const displayDiffZero = isCurrency ? 'Rp 0' : '0';
            return `${displayVal} <span style="color: #94a3b8; font-weight: 500; margin-left: 4px;">(+${displayDiffZero})</span> <span style="color: #94a3b8; font-weight: 600; margin-left: 4px;">(0%)</span>`;
          }
          const prevVal = w.config.series[seriesIndex].data[dataPointIndex - 1];
          if (prevVal === 0) {
            const displayDiffZero = isCurrency ? 'Rp 0' : '0';
            return `${displayVal} <span style="color: #94a3b8; font-weight: 500; margin-left: 4px;">(+${displayDiffZero})</span> <span style="color: #94a3b8; font-weight: 600; margin-left: 4px;">(0%)</span>`;
          }
          const diff = val - prevVal;
          const pctVal = ((val - prevVal) / prevVal) * 100;
          const isUp = pctVal >= 0;
          const formattedPct = isUp ? `+${pctVal.toFixed(2)}%` : `${pctVal.toFixed(2)}%`;
          const displayDiff = isCurrency 
            ? (diff >= 0 ? `+Rp ${diff.toLocaleString('id-ID')}` : `-Rp ${Math.abs(diff).toLocaleString('id-ID')}`)
            : (diff >= 0 ? `+${diff.toLocaleString('id-ID')}` : diff.toLocaleString('id-ID'));
          const diffColor = isUp ? '#3b82f6' : '#f97316';
          const pctColor = isUp ? '#22c55e' : '#ef4444';
          return `${displayVal} <span style="color: ${diffColor}; font-weight: 500; margin-left: 4px;">(${displayDiff})</span> <span style="color: ${pctColor}; font-weight: 600; margin-left: 4px;">(${formattedPct})</span>`;
        }
      },
    },
    legend: {
      position: 'top',
      horizontalAlign: 'center',
      fontFamily: 'Plus Jakarta Sans',
      fontWeight: 600,
      labels: { colors: theme === 'dark' ? '#f8fafc' : '#0f172a' },
    },
  });

  const latestCustomer = customerRaw.Customer[customerRaw.Customer.length - 1];
  const latestOrder    = customerRaw.Order[customerRaw.Order.length - 1];
  const latestOmset    = customerRaw.Omset[customerRaw.Omset.length - 1];

  const prevCustomer = customerRaw.Customer[customerRaw.Customer.length - 2];
  const prevOrder    = customerRaw.Order[customerRaw.Order.length - 2];
  const prevOmset    = customerRaw.Omset[customerRaw.Omset.length - 2];

  const customerDiff = latestCustomer - prevCustomer;
  const orderDiff    = latestOrder - prevOrder;
  const omsetDiff    = latestOmset - prevOmset;

  const customerPct = pct(prevCustomer, latestCustomer);
  const orderPct    = pct(prevOrder, latestOrder);
  const omsetPct    = pct(prevOmset, latestOmset);

  const peaks = getPeakGrowth(customerRaw);

  return (
    <div className="min-h-screen bg-[var(--color-bg-primary)] text-[var(--color-text-primary)] transition-colors duration-300 pb-12">
      {/* Header */}
      <header className="border-b border-[var(--color-border-custom)] bg-white/70 dark:bg-[#0c0b08]/70 backdrop-blur-md sticky top-0 z-45 font-body">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link
              to="/dashboard"
              className="p-2 rounded-full border border-[var(--color-border-custom)] hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors flex items-center justify-center text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"
              aria-label="Back to Dashboard"
            >
              <ArrowLeft size={18} />
            </Link>
            <div>
              <span className="text-xs font-semibold text-[var(--color-accent-primary)] uppercase tracking-wider block">HaloMoment</span>
              <h1 className="font-display font-bold text-lg sm:text-xl md:text-2xl">
                Dashboard Customer & Penjualan
              </h1>
            </div>
          </div>
          <Link to="/"><Button size="sm" variant="ghost">Kembali ke Home</Button></Link>
        </div>
      </header>

      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 sm:mt-10 space-y-6 font-body">
        
        {/* KPI Platform Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          <PlatformCard
            label="Total Customer"
            accentClass="text-violet-500"
            value={latestCustomer}
            changeVal={customerDiff}
            pctVal={customerPct}
            icon={<Users size={20} className="text-violet-500" />}
            bgAccent="bg-violet-50 dark:bg-violet-950/20"
            isCurrency={false}
          />
          <PlatformCard
            label="Total Order"
            accentClass="text-emerald-500"
            value={latestOrder}
            changeVal={orderDiff}
            pctVal={orderPct}
            icon={<ShoppingCart size={20} className="text-emerald-500" />}
            bgAccent="bg-emerald-50 dark:bg-emerald-950/20"
            isCurrency={false}
          />
          <PlatformCard
            label="Total Omset"
            accentClass="text-rose-500"
            value={latestOmset}
            changeVal={omsetDiff}
            pctVal={omsetPct}
            icon={<DollarSign size={20} className="text-rose-500" />}
            bgAccent="bg-rose-50 dark:bg-rose-950/20"
            isCurrency={true}
          />
        </div>

        {/* Charts Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <ChartCard
            title="Customer"
            subtitle="Pertumbuhan Jumlah Customer"
            icon={<Users size={18} className="text-violet-500" />}
            series={customerSeries}
            chartOptions={getChartOptions(['#a855f7'], false)}
            changes={{ Customer: customerChanges.Customer }}
            accentClass="text-violet-500"
            isCurrency={false}
          />
          <ChartCard
            title="Order"
            subtitle="Pertumbuhan Jumlah Orderan"
            icon={<ShoppingCart size={18} className="text-emerald-500" />}
            series={orderSeries}
            chartOptions={getChartOptions(['#10b981'], false)}
            changes={{ Order: customerChanges.Order }}
            accentClass="text-emerald-500"
            isCurrency={false}
          />
          <ChartCard
            title="Omset"
            subtitle="Pertumbuhan Total Omset Penjualan"
            icon={<DollarSign size={18} className="text-rose-500" />}
            series={omsetSeries}
            chartOptions={getChartOptions(['#f43f5e'], true)}
            changes={{ Omset: customerChanges.Omset }}
            accentClass="text-rose-500"
            isCurrency={true}
          />
        </div>

        {/* Peak Growth Highlights */}
        <div className="rounded-2xl border border-[var(--color-border-custom)] bg-[var(--color-bg-card)] p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-6 pb-4 border-b border-[var(--color-border-custom)]">
            <div className="p-2 rounded-xl bg-amber-50 dark:bg-amber-950/20 text-amber-500">
              <Sparkles size={20} />
            </div>
            <div>
              <h3 className="text-base font-bold font-display text-[var(--color-text-primary)]">
                Highlight Pertumbuhan Tertinggi
              </h3>
              <p className="text-xs text-[var(--color-text-secondary)]">
                Analisis metrik customer dan penjualan dengan persentase kenaikan mingguan paling signifikan.
              </p>
            </div>
          </div>
          <CustomerPeakGrowthCard peaks={peaks} theme={theme} />
        </div>

      </main>
    </div>
  );
};
